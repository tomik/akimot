...
void Board::testStepsStructure()
   ...
   * Since kind of lazy method for stepArray maintanence is used - requirements are not that high
   * + every move in the generated moves is also in the structure 
   * - all moves in structure are unique ( no this doesn't have to hold now ) 
   * - every move in the structure is also among generated all moves ( doesn't have to hold as well )
   * */
{

  StepArray oldStepArray;
  uint stepsNum;
  ...
  for ( uint playerIndex = 0; playerIndex < 2; playerIndex++) {
    ...
    stepsNum = generateAllStepsOld(player,oldStepArray,true);  //false == nopushpulls
    ...
    for ( uint i = 0; i < stepsNum; i++ )
      log_() << oldStepArray[i].toString();
    log_() << endl;
    

    for ( uint j = 0; j < stepArrayLen[playerIndex]; j++)
      for ( uint i = 0; i < stepsNum; i++ )                     //check whether is in ref array
        if (stepArray[playerIndex][j] == oldStepArray[i]) 
          oldStepArray[i] = Step(STEP_NO_STEP);    //to check in the end that all moves were found

      
       
    //all moves generated by verified method were found 
    for ( uint i = 0; i < stepsNum; i++ ) {
      assert( oldStepArray[i].stepType_ == STEP_NO_STEP );
    }
  }
      ...
      generatePushPullsFromSquare(neighbour, exclude);  
       ...
       if ( ! nowFrozen && ! frozenBoard_[neighbour]){
         if ( isEmpty){    //single steps && pulls
           ...
           if ( ! ( PIECE(board_[neighbour]) == PIECE_RABBIT && 
...
int Board::generateAllStepsOld(player_t player, StepArray oldStepArray, bool pushPulls) const
{
    int stepsNum;
    int i,j;
